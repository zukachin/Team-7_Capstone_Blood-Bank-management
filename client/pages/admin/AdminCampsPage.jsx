...
export default function AdminCampsPage() {
  ...
  const handleStatusUpdate = async (campId, action) => {
    try {
      console.log("Updating status of camp", campId, "to", action);

      const res = await api.updateCampStatus(campId, action);
      console.log("Response from updateCampStatus:", res);

      // Update UI locally
      setCamps((prevCamps) =>
        prevCamps.map((camp) => {
          if (camp.camp_id === campId) {
            // adjust this based on what backend returns / what status values you use
            const newStatus =
              action === "approve"
                ? "active"
                : action === "reject"
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-6">
      ...
      {camps.length === 0 ? (
        ...
        <div className="space-y-6">
          ...
          {camps.map((camp) => {
            // debug
            console.log("camp:", camp.camp_id, "status:", camp.status);
            return (
              <div
                key={camp.camp_id}
              ...
              >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center mb-3">
                      ...
                      <span className="text-lg font-medium text-gray-700">
                        {camp.organization_name || camp.organizer_name || "Organizer"}
                      </span>
                    </div>
                  </div>
                  <div
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-800">
                        {camp.contact_person || camp.organizer_name || "N/A"}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-gray-100 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  ...
                  {camp.status && camp.status.toLowerCase() === "pending" && (
