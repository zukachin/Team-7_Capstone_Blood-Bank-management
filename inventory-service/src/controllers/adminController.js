...
exports.createAdmin = async (req, res) => {
  try {
    const { admin_name, email = null, password, centre_id = null, state_id = null } = req.body;
    if (!admin_name || !password)
      return res.status(400).json({ message: 'admin_name and password are required' });
    ...
    const insertSql = `
      INSERT INTO admins (admin_name, email, password_hash, role_id, centre_id, state_id)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING admin_id, admin_name, email, role_id, centre_id, state_id, created_at
    `;
    const result = await pool.query(insertSql, [admin_name, email, password_hash, role_id, centre_id, state_id]);
    return res.status(201).json({ admin: result.rows[0] });
  } catch (err) {
    console.error('createAdmin error:', err);
    return res.status(500).json({ message: 'Server error', error: err.message });
...
exports.getAdmins = async (req, res) => {
  try {
    ...
    if (role === 'SuperAdmin') {
      const sql = `
        SELECT a.admin_id, a.admin_name, a.email, a.centre_id, a.created_at, r.role_name, a.state_id, s.state_name
        FROM admins a
        JOIN roles r ON a.role_id = r.role_id
...
exports.getAdminById = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT a.admin_id, a.admin_name, a.email, a.centre_id, a.role_id, a.created_at, a.last_login, a.state_id, s.state_name
       FROM admins a
       LEFT JOIN states s ON a.state_id = s.state_id
       WHERE a.admin_id = $1`,
      [admin_id]
...
exports.updateAdmin = async (req, res) => {
  try {
    ...
    const { role, id: requesterId } = req.user;
    const { admin_name, email, password, centre_id, state_id } = req.body;
    ...
    if (state_id !== undefined) {
      if (role !== 'SuperAdmin') {
        return res.status(403).json({ message: 'Only SuperAdmin can change state assignment' });
      }
    ...
    const sql = `
      ...
      SET ${updates.join(', ')}
      WHERE admin_id = $${idx}
      RETURNING admin_id, admin_name, email, centre_id, role_id, created_at, last_login, state_id
    `;
    params.push(admin_id);
    const result = await pool.query(sql, params);
    return res.status(200).json({ admin: result.rows[0] });
